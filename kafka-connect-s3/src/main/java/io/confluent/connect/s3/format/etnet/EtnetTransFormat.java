...
public class EtnetTransFormat implements Format<S3SinkConnectorConfig, String> {
  private static final Logger log = LoggerFactory.getLogger(EtnetTransFormat.class);
  private final S3Storage storage;

  // DO NOT change this signature, it is required for instantiation via reflection
  public EtnetTransFormat(S3Storage storage) {
    this.storage = storage;
  }

  @Override
  public RecordWriterProvider<S3SinkConnectorConfig> getRecordWriterProvider() {
    return new EtnetTransRecordWriterProvider(storage);
  }
